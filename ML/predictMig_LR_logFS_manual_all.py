# --------------- WITH (all) MANUAL FEATURE SELECTION ----------------------------
...
# use all columns that are max/min for any m
SFS_maxim = data.iloc[:,1:501].idxmax(axis = 1).drop_duplicates() # ALL SFS max columns
SFS_minim = data.iloc[:,1:501].idxmin(axis = 1).drop_duplicates()

FST_maxim = data.iloc[:,501:].idxmax(axis = 1).drop_duplicates()
FST_minim = data.iloc[:,501:].idxmin(axis = 1).drop_duplicates()
...
FS = "\nmanual feature selection: using all max/min columns, and mean and mean $F_{ST}$ and SFS values, columns: " + str(len(data.columns))
fig.suptitle("number of m tested every iteration: " + str(test_size) + " out of " + str(len(data)) + "\ntotal iterations: " + str(cv) + FS)
fig.tight_layout()

fig.savefig("predictMig_LR_logged_manual_FS_all_" + str(cv) +".png")